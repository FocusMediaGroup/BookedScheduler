...
This will be the name of the database in your Booked config file. 'hsp' is the recommended database name.  
...
Import `/database_schema/create-schema.sql` to hsp (or whatever database name was used in the creation process)  
Import `/database_schema/create-data.sql` to hsp (or whatever database name was used in the creation process)  
...
Run or import `/database_schema/full-install.sql` Optionally - run/import `/database_schema/testdata-utf8.sql` to hsp (sample application data will be created with 2 users: admin/password and user/password).  
...
- For Unix, set the path to something similar to <param name="file" value="/tmp/hsp/log\_%s.log" />
